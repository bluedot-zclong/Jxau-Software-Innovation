...
public class VerifyCodeUtils {
	 ...
	 * 对比验证码
	 */
	public static boolean verifyCode(String randomcode) {
		
		Session session = SecurityUtils.getSubject().getSession();
		// 从session中取出验证码(正确的验证码)
		String validateCode = (String) session.getAttribute(VerifyCodeUtils.V_CODE);
		// 输入的验证码和session中的验证进行对比
		if (StringUtils.isAnyBlank(validateCode, randomcode)) {
            return false;
        }
        if (StringUtils.equalsIgnoreCase(validateCode, randomcode)) {
            return true;
        }
        return false;
	}
	 ...
	 * 对比验证码
	 */
	public static boolean verifyCode(String randomcode, String validateCode) {
		// 输入的验证码和session中的验证进行对比
		if (StringUtils.isAnyBlank(validateCode, randomcode)) {
            return false;
        }
