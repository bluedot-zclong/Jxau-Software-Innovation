...
//	}
	 ...
	 * @return
	 */
	public static boolean forgetVerify(String email, String tel) {
		
		if (StringUtils.isAnyBlank(email, tel)) {
			return false;
		}else if(!isMobile(tel) || !isEmail(email)) {
			return false;
		}
		return true;
	 }
	 ...
	 * @return
	 */
	public static boolean vcodeVerify(String vcode, HttpServletRequest req) {
		// 获取session中的验证码
		String validateCode = (String) req.getSession().getAttribute(VerifyCodeUtils.V_CODE);
		// 把验证码去除
		req.getSession().removeAttribute(VerifyCodeUtils.V_CODE);
		if (!VerifyCodeUtils.verifyCode(vcode, validateCode)) {
			return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public static boolean memberVerify(Member member) {
		...
		String memberAcademy = member.getMemberAcademy();
//	    String memberMajor = member.getMemberMajor();
	    String memberClasss = member.getMemberClasss();

	    // 判断是添加老师还是队员
		if(memberRole != null) {
			if (StringUtils.isAnyBlank(memberName, memberAcademy)) {
				return false;
			}
		}else {
			if (StringUtils.isAnyBlank(memberName, memberAcademy, memberClasss)) {
				return false;
			}
		    ...
		    if (!StringUtils.isAnyBlank(memberTel, memberEmail) && !isEmail(memberEmail) && !isTel(memberTel)) {
				return false;
			}
		}
		return true;
	 ...
	 * @return
	 */
	public static boolean teacherVerify(String memberName, String memberAcademy) {
		if (StringUtils.isAnyBlank(memberName, memberAcademy)) {
			return false;
		}
		return true;
